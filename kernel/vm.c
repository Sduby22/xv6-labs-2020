...
static void vmprint_recusive(pagetable_t pgtbl, int depth) {
  for (int i = 0; i != 512; i++) {
    ...
    printf("..%d: pte %p pa %p\n", i, *pte, PTE2PA(*pte));
    if (depth != 2)
      vmprint_recusive((pagetable_t)PTE2PA(*pte), depth+1);
  }
}
...
void
uvminit_kernel(pagetable_t upgtbl, pagetable_t kpgtbl
                , uchar *src, uint sz)
{
  char *mem;
  ...
  mem = kalloc();
  memset(mem, 0, PGSIZE);
  mappages(upgtbl, 0, PGSIZE, (uint64)mem, PTE_W|PTE_R|PTE_X|PTE_U);
  kvmmap_user(kpgtbl, 0, (uint64)mem, PGSIZE, PTE_W|PTE_R|PTE_X);
  memmove(mem, src, sz);
}
...
uint64
uvmalloc_kernel(pagetable_t upgtbl, pagetable_t kpgtbl, uint64 oldsz, uint64 newsz)
{
  char *mem;
  uint64 a;
  ...
  for(a = oldsz; a < newsz; a += PGSIZE){
    mem = kalloc();
    if(mem == 0){
      uvmdealloc_kernel(upgtbl, kpgtbl, a, oldsz);
      return 0;
    }
    memset(mem, 0, PGSIZE);
    if(mappages(upgtbl, a, PGSIZE, (uint64)mem
                  , PTE_W|PTE_X|PTE_R|PTE_U) != 0){
      kfree(mem);
      uvmdealloc_kernel(upgtbl, kpgtbl, a, oldsz);
      return 0;
    }
    kvmmap_user(kpgtbl, a, (uint64)mem, PGSIZE
                  , PTE_W|PTE_X|PTE_R); 
  }
  return newsz;
}
...
uint64
uvmdealloc_kernel(pagetable_t upgtbl, pagetable_t kpgtbl, uint64 oldsz, uint64 newsz)
{
  if(newsz >= oldsz)
    return oldsz;
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(upgtbl, PGROUNDUP(newsz), npages, 1);
    kvmunmap_user(kpgtbl, PGROUNDUP(newsz), npages*PGSIZE);
  }
...
void
uvmfree_kernel(pagetable_t pagetable, uint64 sz)
{
  if(sz > 0)
    kvmunmap_user(pagetable, 0, PGROUNDUP(sz));
...
int
uvmcopy_kernel(pagetable_t old, pagetable_t new, pagetable_t newk, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if (flags & PTE_V && flags & PTE_U)
      flags &= ~PTE_U;
    kvmmap_user(newk, i, (uint64)mem, PGSIZE, flags);
  }
  return 0;
...
void
uvmclear_kernel(pagetable_t upgtbl, pagetable_t kpgtbl, uint64 va)
{
  pte_t *pte;
  
  pte = walk(upgtbl, va, 0);
  if(pte == 0)
    panic("uvmclear");
  *pte &= ~PTE_U;
