...
#define Refindex(x) ((x-KERNBASE)/PGSIZE)
/*
...
{
  if(kmappages(kernel_pagetable, va, sz, pa, perm) != 0)
    panic("kvmmap");
}
...
// resume the flags of the parent.
// remove PTE_COW
// add PTE_W
void resume_flags(pagetable_t pagetable, uint64 sz)
{
  pte_t *pte;
  int i;
